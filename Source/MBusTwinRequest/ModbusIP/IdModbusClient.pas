...
type
  ...
  {$ENDIF}
    ...
    FTimeOut: Cardinal;
    FUnitID: Byte;
    FSendBuffer: TModBusRequestBuffer;
    FReceiveBuffer: TModBusResponseBuffer;
  ...
  public
    property SendBuffer: TModBusRequestBuffer read FSendBuffer;
    property ReceiveBuffer: TModBusResponseBuffer read FReceiveBuffer;
  ...
  {$ENDIF}
    ...
    function WriteRegisters(const RegNo, Blocks: Word; const RegisterData: array of Word): Boolean;
  ...
  published
    property ModbusErrCode: Integer read FModBusErrCode write FModBusErrCode;
...
{$ENDIF}
  ...
  Move(SendBuffer, FSendBuffer, Sizeof(FSendBuffer));
...
{$ENDIF}
  ...
  Move(ReceiveBuffer, FReceiveBuffer, Sizeof(FReceiveBuffer));
...
{ Check if the result has the same function code as the request }
  ...
  begin
    if ((AModBusFunction and $80) = ReceiveBuffer.FunctionCode) then begin
      DoResponseError(AModBusFunction, ReceiveBuffer.MBPData[0], ReceiveBuffer);
      FModBusErrCode := mbecEXCEPTION;
    end
    else begin
...
begin
  ...
  Result := WriteRegisters(RegNo, Length(Buffer), Buffer);
end;
...
function TIdModBusClient.WriteRegisters(const RegNo, Blocks: Word;
...
begin
  ...
  try
    ...
    Result := SendCommand(mbfWriteRegs, RegNo, Blocks, Data);
  finally
    if bNewConnection then
      DisConnect;
  end;
...
begin
  ...
  try
    ...
    Result := SendCommand(mbfWriteCoils, RegNo, Blocks, Data);
  finally
    if bNewConnection then
      DisConnect;
  end;
